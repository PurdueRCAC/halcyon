<?php

return [
	'widget name' => 'News',
	'widget desc' => 'This displays a list of news articles',

	// Misc.
	'news' => 'News',
	'no articles found' => 'No articles found.',
	'no type found' => 'There are no upcoming :type at this time.',
	'updated' => 'Updated',
	'more' => 'see more…',

	// Params
	'params' => [
		'category desc' => 'Select Articles from a specific Category. If no selection, it will show all categories by default.',
		'category' => 'Category',
		'all categories' => '- All Categories -',
		'link title desc' => 'Link the Article titles to Articles.',
		'link title' => 'Linked Titles',
		'number of items desc' => 'The number of Articles to display within this widget. If set to zero, all will be displayed.',
		'number of items' => 'Number of Articles',
		'ordering desc' => 'Select the order in which you want query results presented.',
		'ordering' => 'Order Results',
		'order by published date' => 'Published Date',
		'order by created date' => 'Created Date',
		'order by title' => 'Title',
		'show title desc' => 'Show/hide Article title',
		'show title' => 'Show Article title',
		'show location desc' => 'Show/hide Article location',
		'show location' => 'Show Article location',
		'show image desc' => 'Show/hide the first image found in the article.',
		'show image' => 'Show Article image',
		'blurb length desc' => 'Length, in characters, of a snippet of the article\'s text. Set to zero to hide snippets.',
		'blurb length' => 'Article snippet length',
		'header level' => 'Header Level',
		'header level desc' => 'Select the desired HTML header level for the Article titles.',
		'separator desc' => 'Show separator after last Article',
		'separator' => 'Show last separator',
		'readmore desc' => 'If set to Show, the "Read more..." link will show if Main text has been provided for an Article.',
		'readmore' => '"Read more..." Link',
		'direction' => 'Order Direction',
		'direction desc' => 'Select the order direction in which you want query results presented.',
		'direction new to old' => 'Newest to Oldest',
		'direction old to new' => 'Oldest to Newest',
		'location' => 'Filter by location',
		'location desc' => 'Filter news articles by location.',
		'resource' => 'Resource',
		'resource desc' => 'Limit entries by selected resources.',
	],
];
