...
class IpTest extends Basic
{
	...
	{
		$ip = new \Halcyon\Utility\Ip('192.168.0.1');

		$this->assertTrue($ip->isValid(), 'Basic IPv4 address did not validate');

		$ip = new \Halcyon\Utility\Ip('256.256.256.256');

		$this->assertFalse($ip->isValid(), 'Invalid IPv4 validated as true');

		$ip = new \Halcyon\Utility\Ip('684D:1111:222:3333:4444:5555:6:77');
	...
	{
		$ip = new \Halcyon\Utility\Ip('192.168.255.255');
		$this->assertTrue($ip->isPrivate(), 'Basic IPv4 address did not identify as private');

		$ip = new \Halcyon\Utility\Ip('172.16.0.1');
		$this->assertTrue($ip->isPrivate(), 'Basic IPv4 address did not identify as private');

		$ip = new \Halcyon\Utility\Ip('10.5.20.135');
		$this->assertTrue($ip->isPrivate(), 'Basic IPv4 address did not identify as private');

		$ip = new \Halcyon\Utility\Ip('192.167.0.1');
		$this->assertFalse($ip->isPrivate(), 'Basic IPv4 address did not identify as public');

		$ip = new \Halcyon\Utility\Ip('172.15.255.255');
		$this->assertFalse($ip->isPrivate(), 'Basic IPv4 address did not identify as public');

		$ip = new \Halcyon\Utility\Ip('11.5.20.135');
		$this->assertFalse($ip->isPrivate(), 'Basic IPv4 address did not identify as public');
	}
