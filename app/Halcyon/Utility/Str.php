...
class Str
{
	 ...
	 * @return  string
	 */
	public static function pad($value, $length = 5, $char = 0, $prfx = true)
	{
		while (strlen($value) < $length)
		{
			$value = $prfx ? $char . "$value" : "$value" . $char;
		}
   ...
   */
	...
	/**
	 * Set casing for names
	 *
	 * @param   string  $name
	 * @return  string
	 */
	public static function properCaseNoun($name)
	{
		$uppercase_surname_prefixes = array(
			'mac',
			'mc',
		);
		$lowercase_surname_prefixes = array(
			'al',
			'de',
			'dos',
			'di',
		...
		foreach (explode(' ', $name) as $word)
		{
			...
			if (preg_match('/^[IVX]+$/i', $word))
			{
				$propercase_name .= strtoupper($word);
			}
			else
			{
				$word = strtolower($word);

				foreach ($uppercase_surname_prefixes as $prefix)
				{
					if (substr($word, 0, strlen($prefix)) == $prefix)
					{
						$word = substr($word, 0, strlen($prefix)) . strtoupper(substr($word, strlen($prefix), 1)) . substr($word, strlen($prefix) + 1);
					}
				}

				if (in_array($word, $lowercase_surname_prefixes))
				{
					$propercase_name .= $word;
				}
				else
				{
					$propercase_name .= strtoupper(substr($word, 0, 1)) . substr($word, 1);
				}
			}
		}

		return $propercase_name;
	}
}
