...
class Form
{
	...
	protected function validateField($element, $group = null, $value = null, $input = null)
	{
		...
		if (!$element instanceof SimpleXMLElement)
		{
			return new Exception(trans('form.ERROR_VALIDATE_FIELD'));
		}
		...
		if ($required)
		{
			...
			if (($value === '') || ($value === null))
			{
				...
				else
				{
					if ($element['label'])
					{
						$message = trans($element['label']);
					}
					else
					{
						$message = trans($element['name']);
					}
					$message = trans('form.VALIDATE_FIELD_REQUIRED', ['field' => $message]);
				}
		...
		if ($type = (string) $element['validate'])
		{
			...
			if ($rule === false)
			{
				return new Exception(trans('form.VALIDATE_FIELD_RULE_MISSING', ['type' => $type]));
			}
		...
		if ($valid === false)
		{
			...
			{
				$message = trans('form.VALIDATE_FIELD_INVALID', ['field' => trans((string) $element['label'])]);
			}
	...
	public static function getInstance($name, $data = null, $options = array(), $replace = true, $xpath = false)
	{
		...
		if (!isset($forms[$name]))
		{
			...
			if (empty($data))
			{
				throw new MissingData(trans('form.ERROR_NO_DATA'));
			}
