...
class Toolbar extends Facade
{
	...
	{
		static::getRoot()->appendButton('Standard', 'plus', $alt, $task, $check);
	}*/
	...
	/*public static function deleteList($msg = '', $task = 'remove', $alt = 'JTOOLBAR_DELETE')
	{
		...
		if ($msg)
		{
			$bar->appendButton('Confirm', $msg, 'trash', $alt, $task, true);
		}
		else
		{
			$bar->appendButton('Standard', 'trash', $alt, $task, true);
		}
	}*/
	...
	{
		static::getRoot()->appendButton('Standard', 'save-close', $alt, $task, false);
	}*/
	 ...
	 * @return  void
	 */
	/*public static function preferences($component, $height = '550', $width = '875', $alt = 'global.options', $path = '', $onClose = '')
	{
		...
		static::getRoot()->appendButton(
			...
			'options',
			$alt,
			route('admin.config.module') . '?module=' . $component . '&path=' . $path,
			$width,
			$height,
			$top,
			$left,
