...
class Batch
{
	...
	public static function access()
	{
		...
		$lines = array(
			'<label id="batch-access-lbl" for="batch-access" class="hasTip" title="' . trans('JLIB_HTML_BATCH_ACCESS_LABEL') . '::' . trans('JLIB_HTML_BATCH_ACCESS_LABEL_DESC') . '">',
			trans('JLIB_HTML_BATCH_ACCESS_LABEL'),
			'</label>',
			Access::assetgrouplist(
				'batch[assetgroup_id]', '',
				'class="inputbox"',
				array(
					'title' => trans('JLIB_HTML_BATCH_NOCHANGE'),
					'id'    => 'batch-access'
				)
			)
		);
	...
	public static function item($extension)
	{
		...
		$lines = array(
			...
			trans('JLIB_HTML_BATCH_MENU_LABEL'),
			'</label>',
			'<fieldset id="batch-choose-action" class="combo">',
			'<select name="batch[category_id]" class="inputbox" id="batch-category-id">',
			'<option value="">' . trans('JSELECT') . '</option>',
	...
	public static function language()
	{
		...
		$lines = array(
			'<label id="batch-language-lbl" for="batch-language" class="hasTip" title="' . trans('JLIB_HTML_BATCH_LANGUAGE_LABEL') . '::' . trans('JLIB_HTML_BATCH_LANGUAGE_LABEL_DESC') . '">',
			trans('JLIB_HTML_BATCH_LANGUAGE_LABEL'),
			'</label>',
			'<select name="batch[language_id]" class="inputbox" id="batch-language-id">',
			'<option value="">' . trans('JLIB_HTML_BATCH_LANGUAGE_NOCHANGE') . '</option>',
	...
	public static function user($noUser = true)
	{
		...
		if ($noUser)
		{
			$optionNo = '<option value="0">' . trans('JLIB_HTML_BATCH_USER_NOUSER') . '</option>';
		}
		...
		$lines = array(
			'<label id="batch-user-lbl" for="batch-user" class="hasTip" title="' . trans('JLIB_HTML_BATCH_USER_LABEL') . '::' . trans('JLIB_HTML_BATCH_USER_LABEL_DESC') . '">',
			trans('JLIB_HTML_BATCH_USER_LABEL'),
			'</label>',
			'<select name="batch[user_id]" class="inputbox" id="batch-user-id">',
			'<option value="">' . trans('JLIB_HTML_BATCH_USER_NOCHANGE') . '</option>',
			$optionNo,
