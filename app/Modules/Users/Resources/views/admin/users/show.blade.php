...
@if ($active == 'notes' && auth()->user()->can('view users.notes'))
...
@section('panel')
<nav class="container-fluid mt-4 pt-4">
	<ul id="user-tabs" class="nav flex-column">
		<li class="nav-item<?php if (!$active || $active == 'account') { echo ' active'; } ?>" role="presentation">
			<a href="{{ route('admin.users.show', ['id' => $user->id]) }}" class="nav-link<?php if (!$active || $active == 'account') { echo ' active'; } ?>" id="user-account-tab" aria-controls="user-account" aria-selected="true">Account</a>
		</li>
		<li class="nav-item<?php if ($active == 'attributes') { echo ' active'; } ?>" role="presentation">
			<a href="{{ route('admin.users.show', ['id' => $user->id, 'section' => 'attributes']) }}" class="nav-link<?php if ($active == 'attributes') { echo ' active'; } ?>" id="user-attributes-tab" aria-controls="user-attributes" aria-selected="false">{{ trans('users::users.attributes') }}</a>
		</li>
		@if (auth()->user()->can('view users.notes'))
			<li class="nav-item<?php if ($active == 'notes') { echo ' active'; } ?>" role="presentation">
				<a href="{{ route('admin.users.show', ['id' => $user->id, 'section' => 'notes']) }}" class="nav-link<?php if ($active == 'notes') { echo ' active'; } ?>" id="user-notes-tab" aria-controls="user-notes" aria-selected="false">{{ trans('users::users.notes') }}</a>
			</li>
		@endif
		@foreach ($sections as $k => $section)
			...
			?>
			<li class="nav-item<?php if ($section['active']) { echo ' active'; } ?>" role="presentation">
				<a href="{{ $section['route'] }}" class="nav-link<?php if ($section['active']) { echo ' active'; } ?>" id="user-{{ $k }}-tab" aria-controls="user-{{ $k }}" aria-selected="false">{!! $section['name'] !!}</a>
			</li>
		@endforeach
	</ul>
</nav>
...
<div id="item-form" class="editform">
	...
	<div class="tab-content" id="user-tabs-content">
		...
		@if ($active == 'notes' && auth()->user()->can('view users.notes'))
			<div id="user-notes" aria-labelledby="user-notes-tab">
				<?php
		...
		@foreach ($sections as $k => $section)
			...
			?>
			@if ($section['active'])
				<div id="user-{{ $k }}" aria-labelledby="user-{{ $k }}-tab">
					{!! $section['content'] !!}
				</div>
			@endif
		@endforeach
