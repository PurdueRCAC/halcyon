...
<form action="{{ route('admin.users.index') }}" method="get" name="adminForm" id="adminForm">
	...
	<fieldset id="filter-bar" class="container-fluid">
		<div class="row">
			...
			<div class="col col-xs-12 col-sm-3">
				<label class="sr-only visually-hidden" for="filter-state">{{ trans('users::users.status') }}:</label>
			...
			<div class="col col-xs-12 col-sm-3">
				<label class="sr-only visually-hidden" for="filter-role_id">{{ trans('users::users.roles') }}:</label>
			...
			<div class="col col-xs-12 col-sm-3">
				<label class="sr-only visually-hidden" for="filter-range">{{ trans('users::users.registration date') }}:</label>
		...
		<div class="card mb-4">
			<div class="table-responsive">
				<table class="table table-hover adminlist">
					<caption class="sr-only visually-hidden">{{ trans('users::users.users') }}</caption>
					<thead>
						<tr>
							<th>
								{!! Html::grid('checkall') !!}
						...
						<tr<?php if ($row->trashed()) { echo ' class="trashed"'; } ?>>
							...
							<td class="text-center priority-3">
									<a class="permissions tip" href="{{ route('admin.users.debug', ['id' => $row->id]) }}" data-tip="{{ trans('users::users.debug user') }}">
										...
										<span class="sr-only visually-hidden">{{ trans('users::users.debug user') }}</span>
									</a>
							</div>
							@endif*/ ?>
							<td class="priority-3">
