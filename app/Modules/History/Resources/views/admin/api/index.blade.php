...
<nav class="container-fluid" aria-label="{{ trans('history::history.module sub sections') }}">
	<ul class="nav nav-tabs">
		<li class="nav-item">
			<a class="nav-link active" href="{{ route('admin.history.api') }}">{{ trans('history::history.requests') }}</a>
		</li>
		<li class="nav-item">
			<a class="nav-link" href="{{ route('admin.history.api.stats') }}">{{ trans('history::history.stats') }}</a>
		</li>
	</ul>
</nav>

<form action="{{ route('admin.history.api') }}" method="get" name="adminForm" id="adminForm">
		...
		<div class="card mb-4">
			<div class="table-responsive">
				<table class="table table-hover adminlist">
					<caption class="sr-only visually-hidden">{{ trans('history::history.api') }}</caption>
					<thead>
						<tr>
							<th scope="col">
					...
					@foreach ($rows as $i => $row)
						...
						<tr{!! $cls !!}>
							<td class="priority-5">
								<a href="{{ route('admin.history.api.show', ['id' => $row->id]) }}">
									{{ $row->id }}
								</a>
							</td>
							<td>
							...
							<td class="priority-4">
								@if ($row->datetime)
									<time datetime="{{ $row->datetime->toDateTimeLocalString() }}">{{ $row->datetime->toDateTimeString() }}</time>
								@else
