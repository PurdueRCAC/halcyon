	...
	<div class="wrapper-news">
		...
		<ul class="news-meta text-muted">
			...
			@if ($article->url)
				...
				?>
				<li><span class="fa fa-fw fa-link" aria-hidden="true"></span> <a href="{{ $article->visitableUrl }}">{{ Illuminate\Support\Str::limit($url['host'], 70) . (isset($url['path']) || isset($url['query']) ? ' ...' : '') }}</a></li>
			@endif
			@if ($article->type)
