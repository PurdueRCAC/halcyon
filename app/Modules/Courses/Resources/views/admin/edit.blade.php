					...
					<div class="form-group">
						...
						<select name="type" id="field-type" class="form-control">
							@if ($row->id || count($classes))
								<option value="course"<?php if (!$row->isWorkshop()) { echo ' selected="selected"'; } ?>>{{ trans('courses::courses.course') }}</option>
							@endif
							<option value="workshop"<?php if ($row->isWorkshop()) { echo ' selected="selected"'; } ?>>{{ trans('courses::courses.workshop') }}</option>
						</select>
					</div>
