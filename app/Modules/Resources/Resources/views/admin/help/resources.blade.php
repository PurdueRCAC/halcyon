	...
	<h1>Resource Manager: Resources</h1>
	...
	<p>In this screen you have the ability to look at a list of resources and sort them in different ways. You can also edit and create resources.</p>
	...
	<dl>
		...
		<dt>Name</dt><dd>The full name of the resource.</dd>
		<dt>Role name</dt><dd>A system named used for internal services, etc.</dd>
		<dt>List name</dt><dd>An alternate to the role name. This is used when URLs, etc show one name but internal or 3rd party systems may call it something else.</dd>
		<dt>Type</dt><dd>Types can specify custom data and/or behavior for resources of that type.</dd>
		<dt>State</dt><dd>Whether or not the resource is enabled.</dd>
		<dt>Access</dt><dd>Resources can be limited to users with specific roles. For example, a resource for a test compute cluster can be set to be 'Staff only'.</dd>
		<dt>Subresources</dt><dd>The number of subresources, typically compute cluster partitions.</dd>
		<dt>Ordering</dt><dd>The order in which the resources should be output in front-end listings.</dd>
		<dt>Users</dt><dd>This generates a list of all users that have access to active allocations for that resource.</dd>
	</dl>

	<h2 id="filters">List Filters</h2>

	<p><strong>Search</strong> - In the upper left is a Search field where resources can be found by name, role name, or list name.</p>

	<p>In the upper right, above the column headings, are three drop-down list boxes as shown below.</p>

	<dl>
		<dt>State</dt><dd>Select the desired state (Active or Retired) to limit the list based on state.</dd>
		<dt>Type</dt><dd>Filter by type.</dd>
		<dt>Batch system</dt><dd>Filter by batch system.</dd>
	</dl>
...
@endif
	...
	<ul>
		<li>Click on the name of a resource to view the resource's properties.</li>
