...
<fieldset>
	...
	@foreach ($field->options as $i => $option)
		<div class="form-check">
			<input type="checkbox" class="form-check-input" name="facets[{{ $field->type_id }}][{{ $field->name }}][]" id="facet-{{ $field->name }}-{{ $i }}"<?php if (in_array($option->value, $value)) { echo ' selected="selected"'; } ?> value="{{ $option->value }}" />
