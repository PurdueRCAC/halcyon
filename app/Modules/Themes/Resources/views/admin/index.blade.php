...
<form action="{{ route('admin.themes.store') }}" method="get" name="adminForm" id="adminForm">
	...
	<fieldset id="filter-bar" class="container-fluid">
		<div class="row">
			...
			<div class="col col-md-3">
				<label class="form-label sr-only visually-hidden" for="filter_client_id">{{ trans('themes::themes.type') }}</label>
					...
					@foreach ($rows as $i => $row)
						<tr>
							...
							<td>
								...
								@endif
								@if (!$row->path())
									<div class="text-warning">{{ trans('themes::themes.error.source path not found', ['path' => app_path('Themes/' . $row->element)]) }}</div>
								@endif
							</td>
							<td>
								@if ($canEdit)
