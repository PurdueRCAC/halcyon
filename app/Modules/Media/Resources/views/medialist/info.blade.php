								...
								<div class="media-preview-inner">
									<?php if ($file->isImage()): ?>
										<div class="media-thumb img-preview <?php echo $ext; ?>" title="{{ $file->getFilename() }}">
											...
											--><img src="{{ $file->getUrl() }}" alt="{{ trans('IMAGE_TITLE', ['name' => $file->getFilename(), 'size' => $file->getFormattedSize()]) }}" width="<?php echo ($file->getWidth() < 260) ? $file->getWidth() : '260'; ?>" />
										</div>
									<?php else: ?>
										<div class="media-thumb doc-item <?php echo $ext; ?>" title="{{ $file->getFilename() }}">
											<span class="media-preview-shim"></span><!--
											--><img src="<?php echo $icon; ?>" alt="{{ trans('IMAGE_TITLE', ['name' => $file->getFilename(), 'size' => $file->getFormattedSize()]) }}" width="80" />
										</div>
									<?php endif; ?>
								</div>
							</div>
