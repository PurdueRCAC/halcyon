...
class Cloud extends Fluent
{
	...
	public function tag($id=null)
	{
		if (!$this->_cache['tags.one']
		 || (
				$id !== null
			 && (int) $this->_cache['tags.one']->id != $id
			 && (string) $this->_cache['tags.one']->tag != $this->normalize($id)
			)
		 )
		{
	...
	public function tags($rtrn='', $filters=array(), $clear=false)
	{
		...
		if (isset($filters['order']) && $filters['order'] == 'taggedon')
		{
			$results
				->select($tbl . '.taggedon')
				->join($tbl, $tbl . '.tagid', $results->getTable() . '.id')
				->group('taggedon');
		}
		 ...
		 || isset($filters['label']))
		{
			...
			{
				$results->where($tbl . '.taggerid', '=', (int) $filters['tagger_id']);
			}
			if (isset($filters['scope']) && $filters['scope'])
			{
				$results->where($tbl . '.tbl', '=', (string) $filters['scope']);
			}
			if (isset($filters['scope_id']) && $filters['scope_id'])
			{
				$results->where($tbl . '.objectid', '=', (int) $filters['scope_id']);
			}
			if (isset($filters['label']) && $filters['label'])
			{
				$results->where($tbl . '.label', '=', (string) $filters['label']); // find labeled tags
			}
		}

		if (isset($filters['admin']) && $filters['admin'] !== null)
		{
			$results->where('admin', '=', (int) $filters['admin']);
		}
		if (isset($filters['created_by']) && $filters['created_by'] > 0)
		{
			$results->where('created_by', '=', (int) $filters['created_by']);
		}
		if (isset($filters['modified_by']) && $filters['modified_by'] > 0)
		{
			$results->where('modified_by', '=', $filters['modified_by']);
		}
		if (isset($filters['search']) && $filters['search'])
		{
			...
			$results->select($results->getTableName() . '.*')
				->join($tbl, $tbl . '.tag_id', $results->getTable() . '.id', 'left');

			$results->whereLike($results->getTable() . '.raw_tag', $filters['search'], 1)
				->orWhereLike($results->getTable() . '.tag', $filters['search'], 1)
		...
		switch (strtolower($rtrn))
		{
			case 'count':
				...
				{
					$results->select($results->getTable() . '.id', null, true);
			...
			default:
				$results
					->select($results->getTable() . '.*')
					->group($results->getTable() . '.id');
	...
	public function removeAll($tagger=0)
	{
		...
		$to = Objct::query()
			->where('tbl', '=', $this->_scope)
			->where('objectid', '=', $this->_scope_id);

		if ($tagger)
		{
			$to->where('taggerid', '=', $tagger);
		}

		$tags = array();

		foreach ($to->get() as $row)
		{
			$tags[] = $row->tagid;
		...
		if (count($oldTags) > 0)
		{
			foreach ($oldTags as $tagItem)
			{
				if (!in_array($tagItem->tag, $tagArray))
				{
					...
					$this->remove($tagItem->tag); //, ($admin ? 0 : $tagger_id));
				}
				else
				{
