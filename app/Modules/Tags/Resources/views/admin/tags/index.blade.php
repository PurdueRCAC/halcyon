			...
			<tbody>
				@foreach ($rows as $i => $row)
					<tr>
						@if (auth()->user()->can('delete tags'))
							<td>
								<span class="form-check"><input type="checkbox" name="id[]" id="cb{{ $i }}" value="{{ $row->id }}" class="form-check-input checkbox-toggle" /><label for="cb{{ $i }}"><span class="sr-only">{{ trans('tags::tags.record id', ['id' => $row->id]) }}</span></label></span>
							</td>
						@endif
						<td class="priority-5">
							{{ $row->id }}
