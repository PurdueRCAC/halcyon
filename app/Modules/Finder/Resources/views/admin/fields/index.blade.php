...
@section('toolbar')
	@if ($filters['state'] == 'trashed')
		@if (auth()->user()->can('delete finder'))
			{!! Toolbar::deleteList(trans('global.confirm delete'), route('admin.finder.fields.delete')) !!}
		@endif
	@else
		@if (auth()->user()->can('edit.state finder'))
			{!!
				Toolbar::publishList(route('admin.finder.fields.publish'));
				Toolbar::unpublishList(route('admin.finder.fields.unpublish'));
				Toolbar::spacer();
			!!}
		@endif
		...
		@if (auth()->user()->can('create finder'))
			{!! Toolbar::addNew(route('admin.finder.fields.create')) !!}
		@endif
...
@section('title')
{{ trans('finder::finder.finder') }}: {{ trans('finder::finder.fields') }}
@stop
...
<form action="{{ route('admin.finder.fields') }}" method="get" name="adminForm" id="adminForm" class="form-inline">
	...
	<table class="table table-hover adminlist">
		<caption class="sr-only visually-hidden">{{ trans('finder::finder.fields') }}</caption>
		<thead>
			<tr>
				@if (auth()->user()->can('delete finder.fields'))
					<th>
						{!! Html::grid('checkall') !!}
					</th>
				@endif
				...
				<th scope="col" class="priority-4">
					{!! Html::grid('sort', trans('finder::finder.label'), 'label', $filters['order_dir'], $filters['order']) !!}
				</th>
				<th scope="col" class="priority-4">
			...
			<tr>
				...
				<td class="priority-5">
					@if (auth()->user()->can('edit finder'))
						<a href="{{ route('admin.finder.fields.edit', ['id' => $row->id]) }}">
					@endif
					{{ $row->id }}
					@if (auth()->user()->can('edit finder'))
						</a>
				...
				<td class="priority-4">
					@if ($row->status)
						@if (auth()->user()->can('edit finder'))
							<a class="badge badge-success" href="{{ route('admin.finder.fields.unpublish', ['id' => $row->id]) }}" data-tip="{{ trans('finder::finder.set state to', ['state' => trans('global.unpublished')]) }}">
						@endif
					...
					@else
						@if (auth()->user()->can('edit finder'))
							<a class="badge badge-secondary" href="{{ route('admin.finder.fields.publish', ['id' => $row->id]) }}" data-tip="{{ trans('finder::finder.set state to', ['state' => trans('global.published')]) }}">
						@endif
