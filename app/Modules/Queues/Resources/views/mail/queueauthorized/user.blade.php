...
<?php if ($itar): ?>
**Access to ITAR systems is only possible by public SSH key authentication. You will need to send a public SSH key, in OpenSSH format, to rcac-help@purdue.edu. New public key requests are generally handled within one business day after account creation.**

SSH key-pairs must be generated on the machine you intend to connect to the ITAR cluster from. For Linux and Mac systems you can generate a SSH key-pair by typing the command `ssh-keygen -t rsa -b 2048` into the terminal. You will need to send the public key from the file `~/.ssh/id_rsa.pub` to rcac-help@purdue.edu. For Windows systems using PuTTY, a key-pair can be generated using puttygen. Select SSH-2 RSA with 2048 bits from the parameters and press Generate. 
