...
/*use App\User;
use Halcyon\Listener\OauthClient;
use Halcyon\Auth\Domain;
use Halcyon\Auth\Link;
use Halcyon\Auth\Status;
use Illumnitae\Support\Facades\App;
use Illumnitae\Support\Request;
use Illumnitae\Support\Session;
use Illumnitae\Support\Route;
use Illumnitae\Support\Lang;
use Illumnitae\Support\Config;*/
...
class CILogon //extends OauthClient
{
	...
	public function link($options=array())
	{
		...
		if ((isset($user_id) && $user_id > 0) || (isset($session) && $session))
		{
			...
			$hzad = Domain::getInstance('authentication', 'cilogon', '');
			...
			{
				$hzal = Link::find_or_create('authentication', 'cilogon', null, $id);
				...
				{
					Log::error(sprintf('Ballast\Auth\Link::find_or_create("authentication", "cilogon", null, %s) returned false', $id));
				}
			}
		}
		else
